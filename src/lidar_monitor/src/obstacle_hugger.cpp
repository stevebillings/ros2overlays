...
private:
  ...
  {
    return ((ahead_dist_ == DIST_NAME_NEAR) || (left_dist_ == DIST_NAME_NEAR) || (right_dist_ == DIST_NAME_NEAR));
  }

  bool is_obstacle_far()
  {
    return ((ahead_dist_ == DIST_NAME_FAR) && (left_dist_ == DIST_NAME_FAR) && (right_dist_ == DIST_NAME_FAR));
  }
  ...
  void interpret_laser(sensor_msgs::msg::LaserScan::SharedPtr msg)
  {
    ...
    RCLCPP_INFO(get_logger(), "left: %d; right: %d: ahead: %d (0=near, 1=ok, 2=far)", left_dist_, right_dist_,
                ahead_dist_);
  }
